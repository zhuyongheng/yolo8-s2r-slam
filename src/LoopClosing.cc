            ...
            if(bDetected)
            {
                ...
                if(mbMergeDetected)
                {
                        ...
                        (!mpCurrentKF->GetMap()->isImuInitialized())))
                    {
                        ...
                        Verbose::PrintMess("*I Have to do Merge ", Verbose::VERBOSITY_QUIET);
...
//    Verbose::PrintMess("MERGE: Request Stop Local Mapping", Verbose::VERBOSITY_DEBUG);
    ///// 请求局部建图线程停止
    // 当发出了请求停止的信号后，localmapping线程结合搜到请求信号就执行停止操作，打印local mapping stop
    mpLocalMapper->RequestStop();
    ...
    Verbose::PrintMess("MERGE: Local Map stopped", Verbose::VERBOSITY_NORMAL);
    ...
    //拿到当前关键帧的numTemporalKFs(25)个最佳共视关键帧
    ...
    // 限制while循环次数
//    const int nMaxTries = 3;  原版
    const int nMaxTries = 5;  //new
    int nNumTries = 0;
    ...
    spMergeConnectedKFs.insert(mpMergeMatchedKF);  //add
    // 记录循环次数
    nNumTries = 0;
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        if(pKFi->GetMap() != pCurrentMap)           //add
            Verbose::PrintMess("Other map KF, this should't happen", Verbose::VERBOSITY_DEBUG);  //add
        // 确保这些共视关键帧在当前地图下
    ...
    {   // 当前地图会被更新，老的地图中的重复地图点会被剔除
        ...
        pMergeMap->ChangeId(pCurrentMap->GetId()); //add
    }
