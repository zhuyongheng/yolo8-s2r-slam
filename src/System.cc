...
//    连接服务器
    mpPythonClient = new PythonClient(5000);
//    mpPythonClient->show();
    SetPointerToTrack(mpPythonClient);
...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas)
{
...
cv::Mat System::TrackStereo_RGBD(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &depthmap, const double &timestamp, string filename)
{
    if(mSensor!=STEREO&&mSensor!=RGBD)
    {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo or RGBD." << endl;
        exit(-1);
    }
    // Check mode change
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageStereo_RGBD(imLeft,imRight,depthmap,timestamp,filename);
...
void System::SetPointerToTrack(PythonClient* pPythonClient)
{
